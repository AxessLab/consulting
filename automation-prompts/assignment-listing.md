...
Persistent dedupe shape: top-level scan metadata plus `sources`, keyed by source
name. Each source stores its listing prefix, bare native `seen_ids`, and visible
counts. Do not commit this file.
...
Pipe-separated lines. IDs use the source prefix (`a`, `v`, `c`, `m`, `n`, ...).
