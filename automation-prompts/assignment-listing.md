...
Persistent dedupe shape: a `sources` object keyed by source, with each source
holding its listing prefix, bare native `seen_ids`, and visible counts. Legacy
`seen_keys` / `platforms` memory is accepted only for migration.
...
Platform is implied by the assignment link. Title is a Slack link
