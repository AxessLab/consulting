...
Default sources are Verama (`v`), Chas Partner Network (`c`), and
...
`total_unique_visible`. Legacy `seen_keys`/`platforms` memory can be read, but
new exports use `sources`.
...
`c` prefix; allakonsultuppdrag.se ids use an `a` prefix. Platform is implied by
