...
Persistent dedupe shape: unified `sources` object. Each source stores its prefix,
bare native `seen_ids`, `total_visible`, and `total_unique_visible`.
...
When adding a new source, choose an unused lowercase prefix and register a scanner
