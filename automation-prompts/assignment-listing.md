...
Persistent dedupe shape: unified `sources` object. Each source entry stores the
listing prefix, bare native `seen_ids`, `total_visible`, and
`total_unique_visible`. Do not store prefixed ids in memory.
...
Pipe-separated lines. Listing ids use source prefixes (`a`, `v`, ...). Source is
