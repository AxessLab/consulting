...
- `new_dedupe_keys` — source-prefixed dedupe keys not seen before
...
- `platform_summary` — per-source scan summary for the debug thread
...
single-source `seen_ids` are accepted only for migration.
...
Pipe-separated lines. Listing ids always use the source registry prefix (`a`,
`v`, `c`, `m`, `n`, ...). Source is implied by the assignment link. Title is a
Slack link (`<url|title>`). Put posted date immediately after listing id. Omit
