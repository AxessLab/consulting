...
- **Assignment listing** runs `python scripts/fetch-assignments.py` to scan all
  registered sources in `scripts/assignment_platforms.py`, then the automation
  agent curates matches and runs `scripts/finalize-listing.py` for three-tier
  Slack output. Heuristic hints live in `scripts/assignment_matching.py`;
  matching uses `consultants.yaml`. Dedupe memory is a unified
  `assignment-listing-seen.json` file with `sources.<source_key>.seen_ids` on
  disk plus automation Memory entry **`assignment-listing-seen.json`** for cloud
