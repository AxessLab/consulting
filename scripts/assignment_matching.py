...
def parse_hours_label(assignment: AssignmentRecord) -> str:
    ...
    duration_scope = re.search(r"^(\d{1,3})\s*%$", duration)
    if duration_scope:
        return f"{duration_scope.group(1)}%"
    hours_per_week = re.search(r"\b(\d{1,3})\s*h(?:ours?)?/?\s*(?:week|vecka|v)\b", duration, re.I)
    if hours_per_week:
        return f"{hours_per_week.group(1)} h/week"
...
def parse_client_label(assignment: AssignmentRecord) -> str:
    ...
    for pattern in (r"(?:Kund|Client|End client|Slutkund)\s*:\s*([^\n|]+)",):
