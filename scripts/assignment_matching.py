...
def is_remote(work_mode: str, location: str) -> bool:
    ...
    if re.search(r"\b(?:[1-9]|[1-9]\d)%\s*remote\b", fields):
        return False
    return (
        "100% remote" in fields
        or any(term in fields for term in ("remote", "distans", "fjarrarbete", "fjärrarbete"))
    )
...
def parse_hours_label(assignment: AssignmentRecord) -> str:
    ...
    hours_match = re.search(r"\b(\d{1,2})\s*h(?:ours)?\s*/\s*week\b", duration, re.I)
    if hours_match:
