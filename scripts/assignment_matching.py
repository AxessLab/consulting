...
from assignment_platforms import AssignmentRecord, SOURCE_REGISTRY
...
def is_remote(work_mode: str, location: str) -> bool:
    ...
    if "hybrid" in fields:
        return False
    if re.search(r"\b(?:0|[1-9]\d?)\s*%\s*remote\b", fields):
        return False
    return bool(re.search(r"\bremote\b|\bdistans\b|\bfjarrarbete\b", fields))
...
def detect_role_categories(assignment: AssignmentRecord) -> set[str]:
    ...
    if not assignment.description.strip() and not skill_names(assignment):
        return set()
...
def cross_platform_dedupe(assignments: list[AssignmentRecord]) -> list[AssignmentRecord]:
    ...
    platform_rank = {
        source_key: int(config["dedupe_rank"])
