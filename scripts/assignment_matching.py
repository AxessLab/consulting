...
def is_hybrid(work_mode: str, location: str) -> bool:
    fields = normalize_text(f"{work_mode} {location}")
    percent_remote = re.search(r"\b([1-9]\d?)\s*%\s*remote\b", fields)
    return "hybrid" in fields or bool(percent_remote)
...
def cross_platform_dedupe(assignments: list[AssignmentRecord]) -> list[AssignmentRecord]:
    """Prefer registry order for obvious duplicate roles across sources."""
    by_fingerprint: dict[str, AssignmentRecord] = {}
    platform_rank = {"verama.com": 0, "allakonsultuppdrag.se": 1, "chaspartnernetwork.se": 2}
