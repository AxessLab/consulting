...
def is_remote(work_mode: str, location: str) -> bool:
    ...
    partial_remote = re.search(r"\b([1-9][0-9]?)\s*%\s*remote\b", fields)
    if partial_remote and "100% remote" not in fields and "100 % remote" not in fields:
        return bool(re.search(r"\b(distans|fjarrarbete|fjärrarbete)\b", fields))
    return bool(re.search(r"\b(remote|distans|fjarrarbete|fjärrarbete)\b", fields))
...
def parse_hours_label(assignment: AssignmentRecord) -> str:
    ...
    hours_match = re.search(
        r"\b(\d{1,2})\s*(?:h|hours?|timmar)\s*/?\s*(?:week|vecka|v)\b",
        text,
        re.I,
    )
    if hours_match:
    ...
    if re.search(r"\b(part time|part-time|deltid)\b", text, re.I):
        return "Part time"
    if re.search(r"\b(full time|full-time|heltid)\b", text, re.I):
        return "100%"
...
def parse_client_label(assignment: AssignmentRecord) -> str:
    ...
    title_match = re.search(
        r"\btill\s+([A-ZÅÄÖ][A-Za-zÅÄÖåäö0-9&(). -]{2,60})\s*$",
        assignment.title,
    )
    if title_match:
...
def format_slack_line(match: MatchedAssignment, scan_date: date) -> str:
    ...
    if assignment.work_mode and normalize_text(assignment.work_mode) not in normalize_text(assignment.location):
