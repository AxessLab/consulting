...
def is_remote(work_mode: str, location: str) -> bool:
    ...
    percent_match = re.search(r"\b(\d{1,3})\s*%\s*remote\b", fields)
    if percent_match and int(percent_match.group(1)) < 100:
        return False
    if "hybrid" in fields and "100% remote" not in fields:
        return False
