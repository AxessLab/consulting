...
def is_remote(work_mode: str, location: str) -> bool:
    ...
    remote_percentages = [int(value) for value in re.findall(r"\b(\d{1,3})\s*%\s*remote\b", fields)]
    if remote_percentages and all(value < 100 for value in remote_percentages):
        fields = re.sub(r"\b\d{1,3}\s*%\s*remote\b", "", fields)
    if any(value >= 100 for value in remote_percentages):
        return True
...
def parse_hours_label(assignment: AssignmentRecord) -> str:
    ...
    hours_match = re.search(r"\b(\d{1,2})\s*(?:h|hours)\s*/?\s*(?:week|vecka|v)\b", duration, re.I)
    if hours_match:
    ...
    scope_match = re.search(
        ...
        r"[^%\n]{0,80}?\b(\d{1,3})(?:\s*[–-]\s*(\d{1,3}))?\s*%",
        text,
        re.I,
    )
    if scope_match:
        start = int(scope_match.group(2))
        end = int(scope_match.group(3)) if scope_match.group(3) else None
