...
def is_remote(work_mode: str, location: str) -> bool:
    ...
    if any(value >= 100 for value in remote_percentages):
        return True
    fields_without_partial_remote = re.sub(r"\b\d{1,2}\s*%\s*remote\b", "", fields)
    return any(
...
def parse_hours_label(assignment: AssignmentRecord) -> str:
    ...
    hours = re.search(r"\b(\d{1,3})\s*(?:h|hours?)\s*/\s*(?:week|vecka|v)\b", duration, re.I)
    if hours:
...
def parse_client_label(assignment: AssignmentRecord) -> str:
    ...
    title_match = re.search(r"\btill\s+([A-ZÅÄÖ][A-Za-zÅÄÖåäö0-9&.\- ]{3,60})\b", assignment.title)
    if title_match:
