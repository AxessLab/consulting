...
from assignment_platforms import ACTIVE_PLATFORM_ORDER, AssignmentRecord
...
def parse_hours_label(assignment: AssignmentRecord) -> str:
    ...
    hours_match = re.search(r"\b(\d{1,3})\s*h(?:ours)?/?(?:week|vecka|v)?\b", duration, re.I)
    if hours_match:
...
def cross_platform_dedupe(assignments: list[AssignmentRecord]) -> list[AssignmentRecord]:
    """Prefer sources in the active registry order for obvious duplicate roles."""
    by_fingerprint: dict[str, AssignmentRecord] = {}
    platform_rank = {platform: index for index, platform in enumerate(ACTIVE_PLATFORM_ORDER)}
