...
def is_remote(work_mode: str, location: str) -> bool:
    ...
    if re.search(r"\b100\s*%\s*remote\b", fields):
        return True
    fields = re.sub(r"\b\d{1,2}\s*%\s*remote\b", " ", fields)
...
def parse_hours_label(assignment: AssignmentRecord) -> str:
    ...
    scope_range_match = re.search(
        r"(omfattning|scope|utilization|beläggning|belaggning|engagemang|max)"
        r"[^%\n]{0,60}?(\d{1,3})\s*-\s*(\d{1,3})\s*%",
        text,
        re.I,
    )
    if scope_range_match:
        return f"{scope_range_match.group(2)}-{scope_range_match.group(3)}%"
    ...
    percent_match = re.fullmatch(r"(\d{1,3})\s*%", duration)
    if percent_match:
        return f"{percent_match.group(1)}%"
    hours_match = re.fullmatch(r"(\d{1,3})\s*h(?:ours)?/?(?:week|vecka|v)?", duration, re.I)
    if hours_match:
...
def cross_platform_dedupe(assignments: list[AssignmentRecord]) -> list[AssignmentRecord]:
    """Prefer sources by registry order when the same role appears multiple times."""
