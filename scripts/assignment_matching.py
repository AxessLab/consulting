...
def is_remote(work_mode: str, location: str) -> bool:
    ...
    if re.search(r"\b(?:[1-9]|[1-9]\d)%\s*remote\b", fields) and not re.search(
        r"\b100%\s*remote\b", fields
    ):
        return any(term in fields for term in ("distans", "fjarrarbete", "fjärrarbete"))
...
def parse_hours_label(assignment: AssignmentRecord) -> str:
    ...
    duration_match = re.search(r"\b(\d{1,3})\s*%", duration)
    if duration_match:
        return f"{duration_match.group(1)}%"
    fixed_duration_match = re.search(r"\b(\d{1,2})\s*(?:h|tim|hours?)\s*/?\s*(?:week|vecka|v)\b", duration, re.I)
    if fixed_duration_match:
        return f"{fixed_duration_match.group(1)} h/week"
    ...
    part_time_match = re.search(r"\b(part time|part-time|deltid)\b", text, re.I)
    if part_time_match:
        return "Part time"
...
def parse_client_label(assignment: AssignmentRecord) -> str:
    ...
    for pattern in (
        ...
        r"(?:Till|För)\s+vår\s+kund\s+([A-ZÅÄÖ][A-Za-zÅÄÖåäö0-9&.,\-\s]{2,80}?)(?:\s+söker|\s+soker|\s+letar|[﻿.,])",
    ):
