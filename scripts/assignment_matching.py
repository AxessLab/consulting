...
def is_remote(work_mode: str, location: str) -> bool:
    ...
    if any(term in fields for term in ("distans", "fjarrarbete", "fjärrarbete")):
        return True
    if any(int(value) >= 100 for value in re.findall(r"\b(\d{1,3})\s*%\s*remote\b", fields)):
        return True
    fields_without_partial_remote = re.sub(r"\b\d{1,3}\s*%\s*remote\b", " ", fields)
    return re.search(r"\bremote\b", fields_without_partial_remote) is not None
...
def match_consultants_for_assignment(
    ...
    if not assignment.description.strip() and not assignment.descriptionSummary.strip() and not assignment.skills:
        return "reject", []
...
def parse_hours_label(assignment: AssignmentRecord) -> str:
    ...
    fixed_hours = re.search(
        r"\b(omfattning|scope|utilization|beläggning|belaggning|engagemang|max)[^.\n]{0,80}?(\d{1,4})\s*(h|hours|timmar)\b",
        text,
        re.I,
    )
    if fixed_hours:
        return f"{fixed_hours.group(2)} hours"
