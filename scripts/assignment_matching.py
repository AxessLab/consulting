...
def is_remote(work_mode: str, location: str) -> bool:
    ...
    if re.search(r"\b(ej distans|on-site|onsite|on site)\b", fields):
        return False
    partial_remote = re.search(r"\b([1-9][0-9]?)\s*%\s*remote\b", fields)
    if partial_remote and "hybrid" in fields:
        return False
    if partial_remote and not re.search(r"\b100\s*%\s*remote\b", fields):
        return False
...
def parse_hours_label(assignment: AssignmentRecord) -> str:
    ...
    duration_percent = re.search(r"\b(\d{1,3})\s*%\b", duration)
    if duration_percent:
...
def parse_client_label(assignment: AssignmentRecord) -> str:
    ...
    for pattern in (
        ...
        r"\bTill\s+vår\s+kund\s+([A-ZÅÄÖ][A-Za-zÅÄÖåäö0-9&\s.-]{2,80})\s+söker",
        r"\bTill\s+([A-ZÅÄÖ][A-Za-zÅÄÖåäö0-9&\s.-]{2,80})\s+söker\s+vi",
    ):
    ...
    title_match = re.search(
        r"\btill\s+([A-ZÅÄÖ][A-Za-zÅÄÖåäö0-9&\s.-]{2,80})(?:$|[,|–-])",
        assignment.title,
    )
    if title_match:
