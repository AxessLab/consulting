...
def is_remote(work_mode: str, location: str) -> bool:
    ...
    if percent_remote:
        try:
            if int(percent_remote.group(1)) >= 100:
                return True
        except ValueError:
            pass
...
def format_slack_line(match: MatchedAssignment, scan_date: date) -> str:
    ...
    def known(value: str | None) -> bool:
        return bool(value and normalize_text(value).strip() not in {"unknown", "not stated", "n/a", "-"})
    ...
    if known(assignment.location):
        segments.append(assignment.location)
    if known(assignment.work_mode):
...
def cross_platform_dedupe(assignments: list[AssignmentRecord]) -> list[AssignmentRecord]:
    """Prefer sources according to the active registry for obvious duplicates."""
