...
def is_remote(work_mode: str, location: str) -> bool:
    ...
    percentage_matches = [int(match) for match in re.findall(r"\b(\d{1,3})\s*%\s*remote\b", fields)]
    if percentage_matches:
        return any(percent >= 100 for percent in percentage_matches)
    if re.search(r"\bhybrid\b", fields):
        fields = fields.replace("remote", "")
...
def parse_hours_label(assignment: AssignmentRecord) -> str:
    ...
    duration_scope = re.search(r"^\s*(\d{1,3})\s*%\s*$", duration)
    if duration_scope:
        return f"{duration_scope.group(1)}%"
    hours = re.search(r"^\s*(\d{1,2})\s*h(?:/week|/vecka| per vecka)?\s*$", duration, re.I)
    if hours:
    ...
    unique_matches = list(dict.fromkeys(scope_matches))
    ...
    for pattern in (r"(?:Kund|End client|Slutkund|Client)\s*:\s*([^\n|]+)",):
        ...
        if match:
            ...
            if len(client) > 3 and normalize_text(client) not in {
                ...
                return client
    title_match = re.search(r"\btill\s+([A-ZÅÄÖ][A-Za-zÅÄÖåäö0-9&.\- ]{3,60})$", assignment.title)
    if title_match:
...
def cross_platform_dedupe(assignments: list[AssignmentRecord]) -> list[AssignmentRecord]:
    """Prefer the registry order when the same role appears on multiple sources."""
