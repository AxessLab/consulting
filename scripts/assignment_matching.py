...
def is_remote(work_mode: str, location: str) -> bool:
    ...
    if re.search(r"\b[1-9][0-9]?\s*%\s*remote\b", fields):
        fields = re.sub(r"\b[1-9][0-9]?\s*%\s*remote\b", "", fields)
...
def parse_hours_label(assignment: AssignmentRecord) -> str:
    ...
    hours_match = re.fullmatch(r"(\d{1,3})\s*h(?:/week|/vecka)?", duration, re.I)
    if hours_match:
...
def parse_client_label(assignment: AssignmentRecord) -> str:
    ...
    title_match = re.search(
        r"\btill\s+([A-ZÅÄÖ][A-Za-zÅÄÖåäö]+(?:\s+[A-ZÅÄÖ][A-Za-zÅÄÖåäö]+){0,3})",
        assignment.title,
    )
    if title_match:
...
def cross_platform_dedupe(assignments: list[AssignmentRecord]) -> list[AssignmentRecord]:
    """Collapse obvious source duplicates using the registry preference order."""
