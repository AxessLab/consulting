...
from assignment_platforms import AssignmentRecord, SOURCE_ORDER
...
def parse_hours_label(assignment: AssignmentRecord) -> str:
    ...
    scope_match = re.search(
        r"\b(omfattning|scope|utilization|beläggning|belaggning|engagemang|max)\b"
        r"\s*[:\-]?\s*(?:ca\.?\s*)?(\d{1,3})\s*(?:%|procent)",
        text,
        re.I,
    )
    if scope_match:
    ...
    duration_percent = re.search(r"^\s*(\d{1,3})\s*%\s*$", duration)
    if duration_percent:
        return f"{duration_percent.group(1)}%"
    hours = re.search(r"\b(\d{1,2})\s*(?:h|hours|timmar)\s*/?\s*(?:week|vecka)\b", duration, re.I)
    if hours:
...
def parse_client_label(assignment: AssignmentRecord) -> str:
    ...
    title_match = re.search(
        r"\btill\s+([A-ZÅÄÖ][A-Za-zÅÄÖåäö&\s]+?)\s*$",
        assignment.title,
    )
    if title_match:
...
def cross_platform_dedupe(assignments: list[AssignmentRecord]) -> list[AssignmentRecord]:
    """Prefer sources in registry order when the same role appears on platforms."""
    by_fingerprint: dict[str, AssignmentRecord] = {}
    platform_rank = {source_key: index for index, source_key in enumerate(SOURCE_ORDER)}
