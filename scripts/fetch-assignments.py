...
from listing_memory import DEFAULT_MEMORY_PATH, build_memory_payload, load_memory, seen_ids_by_source
...
def prepare_candidates(
    *,
    ...
    seen_source_ids = seen_ids_by_source(memory_data)
    ...
    new_raw_assignments = [
        ...
        if assignment.platform in successful_platforms
        and assignment.source_id not in seen_source_ids.get(assignment.platform, set())
    ]
    report_pool = cross_platform_dedupe(new_raw_assignments)
    deduped_assignments = cross_platform_dedupe(raw_assignments)
    new_assignments = [
        assignment
        for assignment in report_pool
        if assignment.source_id not in seen_source_ids.get(assignment.platform, set())
    ]
    ...
    return {
        ...
        "stats": {
            ...
            "total_unique_after_cross_source_dedupe": len(deduped_assignments),
            ...
            "new_ids_by_source": {
                platform_id: sum(
                    1
                    for assignment in raw_assignments
                    if assignment.platform == platform_id
                    and assignment.source_id not in seen_source_ids.get(platform_id, set())
                )
