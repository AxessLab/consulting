...
def build_source_stats(
    ...
    previous_seen: dict[str, set[str]],
    ...
    for result in platform_results:
        visible_ids = ids_by_source.get(result.platform, set())
        stats[result.platform] = {
            "status": result.status,
            "total_visible": result.count,
            "total_unique_visible": len(visible_ids),
            "new_ids": len(visible_ids - previous_seen.get(result.platform, set())),
        }
    return stats
...
def prepare_candidates(
    *,
    ...
    previous_seen_by_source = seen_ids_by_source(previous_memory)

    raw_assignments, platform_results = scan_platforms(
        platform_ids,
        max_pages=max_pages,
        headless=headless,
        seen_ids_by_platform=previous_seen_by_source,
        scan_date=scan_date,
    )
    ...
    source_stats = build_source_stats(raw_assignments, platform_results, previous_seen_by_source)
    ...
    return {
        ...
        "stats": {
            ...
            "new_ids_by_source": {
                key: value["new_ids"] for key, value in source_stats.items()
            },
