...
def count_new_by_source(
    ...
    for assignment in assignments:
        if assignment.source_id in seen_ids_by_source.get(assignment.source_key, set()):
            continue
        new_by_source.setdefault(assignment.source_key, set()).add(assignment.source_id)
    return {source_key: len(ids) for source_key, ids in sorted(new_by_source.items())}
...
def prepare_candidates(
    *,
    ...
    seen_ids_by_source = source_seen_ids(memory_data)
    ...
    return {
        ...
        "stats": {
            ...
            "new_ids_by_source": count_new_by_source(raw_assignments, seen_ids_by_source),
