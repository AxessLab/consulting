...
def dedupe_within_source(assignments: list[AssignmentRecord]) -> list[AssignmentRecord]:
...
def count_by_source(assignments: list[AssignmentRecord]) -> dict[str, int]:
    counts: dict[str, int] = {}
    for assignment in assignments:
...
def prepare_candidates(
    *,
    ...
    seen_ids_by_source = collect_seen_ids_by_source(memory_data)

    raw_assignments, platform_results = scan_platforms(
        platform_ids,
        max_pages=max_pages,
        headless=headless,
        seen_ids_by_platform=seen_ids_by_source,
        scan_date=datetime.combine(scan_date, datetime.min.time(), tzinfo=UTC),
    )
    source_unique_assignments = dedupe_within_source(raw_assignments)
    deduped_assignments = cross_platform_dedupe(source_unique_assignments)
    new_assignments = [
        assignment
        for assignment in deduped_assignments
        if assignment.dedupe_key not in seen_keys
    ]
    new_source_unique_assignments = [
        assignment
        for assignment in source_unique_assignments
        if assignment.dedupe_key not in seen_keys
    ]
    ...
    memory_update = build_memory_payload(
        assignments=source_unique_assignments,
    ...
    return {
        ...
        "stats": {
            ...
            "new_ids_by_source": count_by_source(new_source_unique_assignments),
            "visible_by_source": {
                result.platform: result.count for result in platform_results
            },
            "unique_visible_by_source": count_by_source(source_unique_assignments),
