...
def per_source_new_counts(
    ...
    for assignment in assignments:
        visible_ids.setdefault(assignment.source_key, set()).add(assignment.source_id)
    return {
        source_key: len(source_ids - seen_by_source.get(source_key, set()))
        for source_key, source_ids in visible_ids.items()
    }
...
def prepare_candidates(
    *,
    ...
    source_seen_ids = seen_ids_by_source(memory_data)
    ...
    return {
        ...
        "stats": {
            ...
            "new_ids": sum(per_source_new_counts(raw_assignments, source_seen_ids).values()),
            "new_ids_by_source": per_source_new_counts(raw_assignments, source_seen_ids),
