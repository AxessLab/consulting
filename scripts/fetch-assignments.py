...
def prepare_candidates(
    *,
    ...
    seen_ids_by_platform = source_seen_ids_by_platform(memory_data)

    raw_assignments, platform_results = scan_platforms(
        platform_ids,
        max_pages=max_pages,
        headless=headless,
        seen_ids_by_platform=seen_ids_by_platform,
        scan_date=datetime.combine(scan_date, datetime.min.time()),
    )
    ...
    for assignment in raw_assignments:
        if assignment.source_id not in seen_ids_by_platform.get(assignment.platform, set()):
