...
def prepare_candidates(
    *,
    ...
    seen_keys = {
        ...
        for source_key, seen_ids in seen_ids_by_source.items()
    ...
    previous_sources = previous_memory.get("sources", {}) if isinstance(previous_memory, dict) else {}
    for result in platform_results:
        if result.status == "ok":
            continue
        previous_state = previous_sources.get(result.platform)
        if isinstance(previous_state, dict):
            memory_update["sources"][result.platform] = previous_state
    new_ids_by_source: dict[str, int] = {}
    for assignment in raw_assignments:
        if assignment.source_id not in seen_ids_by_source.get(assignment.platform, set()):
